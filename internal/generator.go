...
func GetTranslationImpl(data TomlData) ([]byte, error) {
...
func WriteBaseTranslation(baseTranslation TomlData, outputDir string) ([]byte, error) {
	var sb strings.Builder
	sb.WriteString("// Code generated by simple-translate; DO NOT EDIT.\n\n")
	sb.WriteString("package translations\n\n")
...
func WriteTranslator(allLangs []string, outputDir string, baseTranslation TomlData) error {
	filename := filepath.Join(outputDir, "translator.go")
	...
	sectionNameToType := make(map[string]string)
	for sectionKey := range baseTranslation.sections {
		sectionName := toPublicName(sectionKey)
	...
	for rootKey := range baseTranslation.root {
